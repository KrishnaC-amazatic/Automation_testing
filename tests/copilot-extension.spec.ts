/**
 * Guardrail Sentinel — Playwright Automation Test (Microsoft Copilot)
 *
 * Mirrors claude-extension.spec.ts but targets Microsoft Copilot instead of Claude.
 * Shares the same 28 TEST_PROMPTS and produces a matching report file.
 *
 * Run standalone:  npx playwright test copilot --reporter=list
...
// Microsoft Copilot selectors (covers multiple UI versions)
const COPILOT_INPUT =
  'textarea#userInput, ' +
...
// ---------------------------------------------------------------------------
// Helper — detect Cloudflare "Verification failed" card via DOM text scan
// ---------------------------------------------------------------------------
async function isCloudflareFailed(page: Page): Promise<boolean> {
  return page.evaluate(() => {
    const bodyText = document.body?.innerText ?? '';
    if (bodyText.includes('Verification failed')) return true;
    // Also catch Turnstile iframes Copilot may inject
    if (document.querySelector('iframe[src*="challenges.cloudflare.com"]')) return true;
    return false;
  }).catch(() => false);
}

// Navigate to a brand-new Copilot conversation and wait for the input.
// Called before every prompt to ensure a clean chat context (no stale CF token).
async function openFreshConversation(page: Page, label: string): Promise<void> {
  await page.goto('https://copilot.microsoft.com/', { waitUntil: 'commit', timeout: 30_000 })
    .catch(e => console.log(`[COPILOT]${label} Nav error: ${String(e).split('\n')[0]}`));
  // Wait for input — if not found within 15 s the page may still be usable
  await page.waitForSelector(COPILOT_INPUT, { timeout: 15_000 }).catch(() => {});
...
test.beforeAll(async () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [COPILOT] STEP 1: Launching Chrome               │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  context.on('serviceworker', (worker) => {
    ...
    if (url.startsWith('chrome-extension://') && !extensionId) {
      ...
      console.log('[COPILOT][SW event] Extension ID:', extensionId);
    }
  });

  await context.addInitScript(() => {
    Object.defineProperty(navigator, 'webdriver', { get: () => undefined });
    // Mask additional automation indicators checked by Cloudflare
    Object.defineProperty(navigator, 'plugins', { get: () => [1, 2, 3, 4, 5] });
    Object.defineProperty(navigator, 'languages', { get: () => ['en-US', 'en'] });
    (window as any).chrome = { runtime: {} };
  });

  console.log('[COPILOT][Step 1] Chrome launched.');
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [COPILOT] STEP 2: Loading extension              │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  if (alreadyLoaded) {
    console.log('[COPILOT][Step 2] Extension already loaded — skipping Load Unpacked.');
  } else {
    console.log('[COPILOT][Step 2] Enabling Developer Mode...');
    ...
    console.log('[COPILOT][Step 2] Clicking "Load unpacked"...');
    ...
    if (!clicked) {
      console.log('[COPILOT][Step 2] WARNING: Could not find "Load unpacked" button.');
    } else {
      console.log('[COPILOT][Step 2] "Load unpacked" clicked — handling file dialog...');
      const psScript = [
      ...
      try {
        ...
        console.log('[COPILOT][Step 2] File dialog handled.');
      } catch (err) {
        console.log('[COPILOT][Step 2] File dialog error:', String(err).split('\n')[0]);
      } finally {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [COPILOT] STEP 3: Verifying extension & ID       │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  if (idFromDom) {
    ...
    console.log('[COPILOT][Step 3] ✓ Extension loaded — ID:', extensionId);
  } else if (extensionId) {
    console.log('[COPILOT][Step 3] ✓ Extension ID from SW event:', extensionId);
  } else {
    const sw = context.serviceWorkers().find(w => w.url().startsWith('chrome-extension://'));
    if (sw) {
      extensionId = sw.url().split('/')[2];
      console.log('[COPILOT][Step 3] ✓ Extension ID from SW list:', extensionId);
    } else {
      console.log('[COPILOT][Step 3] ✗ Extension NOT found in chrome://extensions.');
    }
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [COPILOT] STEP 4: Sign in through extension popup│');
  console.log('└─────────────────────────────────────────────────┘');

  let popupPage: Page | null = null;

  if (!extensionId) {
    console.log('[COPILOT][Step 4] Skipped — extension ID unknown.');
  } else {
    const popupUrl = `chrome-extension://${extensionId}/popup/popup.html`;
    console.log('[COPILOT][Step 4] Opening popup:', popupUrl);
    try {
      ...
      console.log('[COPILOT][Step 4] ✓ Popup opened.');
      ...
      if (alreadySignedIn) {
        console.log('[COPILOT][Step 4] ✓ Already signed in.');
      } else {
        console.log('[COPILOT][Step 4] Not signed in — automating sign-in...');
        const popupPreview = await popupPage
          .evaluate(() => (document.body?.innerText ?? '').replace(/\n+/g, ' ').substring(0, 200))
          .catch(() => '');
        console.log('[COPILOT][Step 4] Popup content:', popupPreview);
        ...
        console.log('[COPILOT][Step 4] Clicked Sign In — waiting for auth tab...');
        ...
        try {
          ...
          console.log('[COPILOT][Step 4] Auth page URL:', authPage.url());
        } catch {
          authPage = popupPage;
          await authPage.waitForTimeout(3000);
          console.log('[COPILOT][Step 4] Auth in popup page:', authPage.url());
        }
        ...
        for (const sel of emailSelectors) {
          ...
          if (visible) {
            ...
            console.log('[COPILOT][Step 4] Filled email using selector:', sel);
            emailFilled = true;
            break;
          }
        }
        if (!emailFilled) {
          ...
          console.log('[COPILOT][Step 4] Filled email via JS evaluate (fallback).');
        }
        ...
        console.log('[COPILOT][Step 4] Credentials submitted — waiting for redirect...');
        ...
        console.log(signedIn
          ? '[COPILOT][Step 4] ✓ Signed in successfully.'
          : '[COPILOT][Step 4] ✗ Sign-in may have failed — Sentinel will run unauthenticated.');
      }
    } catch (err) {
      console.log('[COPILOT][Step 4] Error:', String(err).split('\n')[0]);
    }
    await popupPage?.close().catch(() => {});
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 5 — Log in to Microsoft Copilot (copilot.microsoft.com)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [COPILOT] STEP 5: Log in to Microsoft Copilot   │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  console.log('[COPILOT][Step 5] Navigating to Microsoft Copilot...');
  await copilotPage.goto('https://copilot.microsoft.com/', { waitUntil: 'commit', timeout: 60_000 })
    .catch(e => console.log('[COPILOT][Step 5] Navigation error:', String(e).split('\n')[0]));

  await copilotPage.waitForTimeout(3000);
  console.log('[COPILOT][Step 5] Current URL:', copilotPage.url());

  // Check if already logged in (Copilot input visible)
  const alreadyLoggedIn = await copilotPage
    .waitForSelector(COPILOT_INPUT, { timeout: 8_000 })
    .then(() => true)
    .catch(() => false);

  if (alreadyLoggedIn) {
    console.log('[COPILOT][Step 5] ✓ Already logged in to Microsoft Copilot.');
  } else {
    console.log('[COPILOT][Step 5] Microsoft Copilot login required...');

    // Copilot may show a "Sign in" button
    const signInBtn = copilotPage.locator(
    ...
    if (signInVisible) {
      console.log('[COPILOT][Step 5] Clicking Sign in...');
      await signInBtn.click();
      await copilotPage.waitForTimeout(2000);
    }

    console.log('[COPILOT][Step 5] URL after click:', copilotPage.url());
    console.log('[COPILOT][Step 5]');
    console.log('[COPILOT][Step 5] ════ ACTION REQUIRED ════════════════════════════════════');
    console.log('[COPILOT][Step 5]  Microsoft Copilot sign-in page is open in the Chrome window.');
    console.log('[COPILOT][Step 5]  Please log in with your Microsoft account.');
    console.log('[COPILOT][Step 5]  Test continues automatically once the Copilot input is visible.');
    console.log('[COPILOT][Step 5] ════════════════════════════════════════════════════════');
    console.log('[COPILOT][Step 5] Waiting up to 2 minutes for Copilot to become ready...');

    await copilotPage
      .waitForSelector(COPILOT_INPUT, { timeout: 120_000 })
      .then(() => console.log('[COPILOT][Step 5] ✓ Copilot login complete — input visible.'))
      .catch(() => console.log('[COPILOT][Step 5] Copilot login not detected — tests may fail.'));
  }

  sharedPage = copilotPage;

  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 6 — Verify extension is active on Copilot (__SENTINEL_INJECTED__)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [COPILOT] STEP 6: Verifying extension active     │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  console.log(`[COPILOT][Step 6] __SENTINEL_INJECTED__ : ${step6Injected}`);
  console.log(`[COPILOT][Step 6] fetch() length        : ${fetchLen}`);
  if (sentinelBootLogs.length) console.log('[COPILOT][Step 6] Sentinel logs:', sentinelBootLogs.slice(0, 3));
  else                         console.log('[COPILOT][Step 6] No Sentinel console logs captured.');

  if (step6Injected) {
    console.log('[COPILOT][Step 6] ✓ Extension is active on Microsoft Copilot.');
  } else {
    console.log('[COPILOT][Step 6] ✗ __SENTINEL_INJECTED__ = false — content script not running.');
  }

  console.log('\n[COPILOT][Setup] All steps complete — starting tests.\n');
});
...
// ---------------------------------------------------------------------------
// TEST 1 — Verify extension is active on Microsoft Copilot
// ---------------------------------------------------------------------------

test('[Copilot] 1 - Extension is active on Microsoft Copilot (__SENTINEL_INJECTED__)', async () => {
  ...
  console.log('\n[COPILOT][Test 1] STEP 7 — Navigating to Microsoft Copilot...');
  await page.goto('https://copilot.microsoft.com/', { waitUntil: 'commit', timeout: 90_000 });
  await page.waitForSelector(COPILOT_INPUT, { timeout: 60_000 });
  ...
  console.log(`[COPILOT][Test 1] STEP 8 — __SENTINEL_INJECTED__: ${injected}  |  fetch hooked: ${fetchHooked}`);
  if (sentinelLogs.length) console.log('[COPILOT][Test 1] Sentinel logs:', sentinelLogs.slice(0, 5));

  expect(
    injected,
    '__SENTINEL_INJECTED__ is false — extension not active on Microsoft Copilot.',
  ).toBe(true);

  console.log('[COPILOT][Test 1] PASS — Sentinel extension is active on Microsoft Copilot.');
});

// ---------------------------------------------------------------------------
// TEST 2 — Enter 28 prompts on Copilot → validate Sentinel decision
// ---------------------------------------------------------------------------

test('[Copilot] 2 - Sentinel intercepts prompt on Copilot (WARN / BLOCK / REDACT / ALLOW)', async () => {
  ...
  // Navigate to Copilot once for the SW-ready check, then each prompt opens its own fresh conversation.
  await page.goto('https://copilot.microsoft.com/', { waitUntil: 'commit', timeout: 90_000 });
  await page.waitForSelector(COPILOT_INPUT, { timeout: 60_000 });
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    console.log(`[COPILOT][Test 2] Waiting 30 s for Sentinel to initialise (attempt ${attempt}/3)...`);
    await page.waitForTimeout(30_000);
    swReady = await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__).catch(() => false);
    console.log(`[COPILOT][Test 2] Sentinel SW ready: ${swReady}`);
    if (swReady) break;
    if (attempt < 3) {
      console.log('[COPILOT][Test 2] SW not ready — reloading page to trigger policy re-sync...');
      await page.reload({ waitUntil: 'commit', timeout: 60_000 });
      await page.waitForSelector(COPILOT_INPUT, { timeout: 30_000 }).catch(() => {});
    }
  }
  if (!swReady) {
    console.log('[COPILOT][Test 2] WARNING: Sentinel SW still not ready after 3 attempts — results may be affected.');
  }
  ...
  async function typeAndSubmit(p: Page, text: string): Promise<boolean> {
    const inp = p.locator(COPILOT_INPUT).first();
    ...
    try {
      const btn = p.locator(COPILOT_SEND).first();
      ...
      console.log('[COPILOT][Test 2] Submit error:', String(err).split('\n')[0]);
      return false;
    }
  }
  ...
  for (let i = 0; i < TEST_PROMPTS.length; i++) {
    ...
    console.log(`\n${'─'.repeat(60)}`);
    console.log(`[COPILOT][Test 2] Prompt ${i + 1} / ${TEST_PROMPTS.length}`);
    console.log(`[COPILOT][Test 2] "${prompt.substring(0, 70)}..."`);
    ...
    console.log(`[COPILOT][Test 2] Sentinel active: ${await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__)}`);

    // Type and submit in the existing chat session
    const submitted = await typeAndSubmit(page, prompt);
    if (!submitted) {
      console.log('[COPILOT][Test 2] Could not submit — skipping prompt.');
      results.push({ index: i + 1, category, expected, decision: 'SKIP', detectedCategory: '', prompt, status: 'SKIP' });
      continue;
    }

    // ── CF guard: poll for up to 8 s right after submit ───────────────────
    // Only open a new conversation if Cloudflare actually fires — otherwise
    // all 28 prompts stay in the same chat thread.
    let cfDetected = false;
    for (let poll = 0; poll < 4; poll++) {
      await page.waitForTimeout(2_000);
      cfDetected = await isCloudflareFailed(page);
      if (cfDetected) break;
    }

    if (cfDetected) {
      console.log('[COPILOT][Test 2] ⚠ CF "Verification failed" — opening fresh conversation and retrying...');
      await openFreshConversation(page, '[Test 2][CF retry]');
      await page.evaluate(() => {
        document.getElementById('sentinel-block-banner')?.remove();
        document.getElementById('sentinel-overlay-root')?.remove();
        document.getElementById('sentinel-redact-toast')?.remove();
      });
      const retried = await typeAndSubmit(page, prompt);
      if (!retried) {
        results.push({ index: i + 1, category, expected, decision: 'SKIP', detectedCategory: '', prompt, status: 'SKIP' });
        continue;
      }
    ...
    // Wait for Sentinel decision (Sentinel intercepts before Copilot responds)
    ...
    console.log(`[COPILOT][Test 2] Prompt ${i + 1} → Decision: ${decision}  Det.Category: ${detectedCategory || '(none)'}  [${status}]`);
  ...
  reportLines.push(`  Guardrail Sentinel — Microsoft Copilot Test Report`);
  ...
  const reportFile = path.join(reportDir, `copilot_${fileTs}.txt`);
  fs.writeFileSync(reportFile, reportText, 'utf8');
  console.log(`[COPILOT][Test 2] Report saved → ${reportFile}`);
  console.log(`[COPILOT][Test 2] Total Sentinel API calls: ${sentinelApiCalls.length}`);

  const extensionActive = await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__).catch(() => false);
  expect(extensionActive, 'Sentinel extension not active on Microsoft Copilot — content script not running.').toBe(true);

  console.log('[COPILOT][Test 2] Keeping browser open for 60 s to review the Sentinel results...');
