/**
 * Guardrail Sentinel — Playwright Automation Test (DeepSeek)
 *
 * Mirrors copilot-extension.spec.ts but targets DeepSeek Chat instead.
 * Shares the same 28 TEST_PROMPTS and produces a matching report file.
 *
 * Run standalone:  npx playwright test deepseek --reporter=list
...
// ---------------------------------------------------------------------------
// Navigate to a brand-new DeepSeek conversation and wait for the input.
...
async function openFreshConversation(page: Page, label: string): Promise<void> {
  await page.goto('https://chat.deepseek.com/', { waitUntil: 'commit', timeout: 30_000 })
    .catch(e => console.log(`[DEEPSEEK]${label} Nav error: ${String(e).split('\n')[0]}`));
  await page.waitForSelector(DEEPSEEK_INPUT, { timeout: 15_000 }).catch(() => {});
...
test.beforeAll(async () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [DEEPSEEK] STEP 1: Launching Chrome              │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  context.on('serviceworker', (worker) => {
    ...
    if (url.startsWith('chrome-extension://') && !extensionId) {
      ...
      console.log('[DEEPSEEK][SW event] Extension ID:', extensionId);
    }
  });
  ...
  console.log('[DEEPSEEK][Step 1] Chrome launched.');
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [DEEPSEEK] STEP 2: Loading extension             │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  if (alreadyLoaded) {
    console.log('[DEEPSEEK][Step 2] Extension already loaded — skipping Load Unpacked.');
  } else {
    console.log('[DEEPSEEK][Step 2] Enabling Developer Mode...');
    ...
    console.log('[DEEPSEEK][Step 2] Clicking "Load unpacked"...');
    ...
    if (!clicked) {
      console.log('[DEEPSEEK][Step 2] WARNING: Could not find "Load unpacked" button.');
    } else {
      console.log('[DEEPSEEK][Step 2] "Load unpacked" clicked — handling file dialog...');
      const psScript = [
      ...
      try {
        ...
        console.log('[DEEPSEEK][Step 2] File dialog handled.');
      } catch (err) {
        console.log('[DEEPSEEK][Step 2] File dialog error:', String(err).split('\n')[0]);
      } finally {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [DEEPSEEK] STEP 3: Verifying extension & ID      │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  if (idFromDom) {
    ...
    console.log('[DEEPSEEK][Step 3] ✓ Extension loaded — ID:', extensionId);
  } else if (extensionId) {
    console.log('[DEEPSEEK][Step 3] ✓ Extension ID from SW event:', extensionId);
  } else {
    const sw = context.serviceWorkers().find(w => w.url().startsWith('chrome-extension://'));
    if (sw) {
      extensionId = sw.url().split('/')[2];
      console.log('[DEEPSEEK][Step 3] ✓ Extension ID from SW list:', extensionId);
    } else {
      console.log('[DEEPSEEK][Step 3] ✗ Extension NOT found in chrome://extensions.');
    }
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [DEEPSEEK] STEP 4: Sign in through extension popup│');
  console.log('└─────────────────────────────────────────────────┘');

  let popupPage: Page | null = null;

  if (!extensionId) {
    console.log('[DEEPSEEK][Step 4] Skipped — extension ID unknown.');
  } else {
    const popupUrl = `chrome-extension://${extensionId}/popup/popup.html`;
    console.log('[DEEPSEEK][Step 4] Opening popup:', popupUrl);
    try {
      ...
      console.log('[DEEPSEEK][Step 4] ✓ Popup opened.');
      ...
      if (alreadySignedIn) {
        console.log('[DEEPSEEK][Step 4] ✓ Already signed in.');
      } else {
        console.log('[DEEPSEEK][Step 4] Not signed in — automating sign-in...');
        const popupPreview = await popupPage
          .evaluate(() => (document.body?.innerText ?? '').replace(/\n+/g, ' ').substring(0, 200))
          .catch(() => '');
        console.log('[DEEPSEEK][Step 4] Popup content:', popupPreview);
        ...
        console.log('[DEEPSEEK][Step 4] Clicked Sign In — waiting for auth tab...');
        ...
        try {
          ...
          console.log('[DEEPSEEK][Step 4] Auth page URL:', authPage.url());
        } catch {
          authPage = popupPage;
          await authPage.waitForTimeout(3000);
          console.log('[DEEPSEEK][Step 4] Auth in popup page:', authPage.url());
        }
        ...
        for (const sel of emailSelectors) {
          ...
          if (visible) {
            ...
            console.log('[DEEPSEEK][Step 4] Filled email using selector:', sel);
            emailFilled = true;
            break;
          }
        }
        if (!emailFilled) {
          ...
          console.log('[DEEPSEEK][Step 4] Filled email via JS evaluate (fallback).');
        }
        ...
        console.log('[DEEPSEEK][Step 4] Credentials submitted — waiting for redirect...');
        ...
        console.log(signedIn
          ? '[DEEPSEEK][Step 4] ✓ Signed in successfully.'
          : '[DEEPSEEK][Step 4] ✗ Sign-in may have failed — Sentinel will run unauthenticated.');
      }
    } catch (err) {
      console.log('[DEEPSEEK][Step 4] Error:', String(err).split('\n')[0]);
    }
    await popupPage?.close().catch(() => {});
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 5 — Log in to DeepSeek Chat (chat.deepseek.com)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [DEEPSEEK] STEP 5: Log in to DeepSeek Chat      │');
  console.log('└─────────────────────────────────────────────────┘');

  const deepseekPage = await context.newPage();
  const sentinelBootLogs: string[] = [];
  deepseekPage.on('console', (msg) => {
    const t = msg.text();
    if (t.toLowerCase().includes('sentinel')) sentinelBootLogs.push(t);
  });

  console.log('[DEEPSEEK][Step 5] Navigating to DeepSeek Chat...');
  await deepseekPage.goto('https://chat.deepseek.com/', { waitUntil: 'commit', timeout: 60_000 })
    .catch(e => console.log('[DEEPSEEK][Step 5] Navigation error:', String(e).split('\n')[0]));

  await deepseekPage.waitForTimeout(3000);
  console.log('[DEEPSEEK][Step 5] Current URL:', deepseekPage.url());

  // Check if already logged in (DeepSeek input visible)
  const alreadyLoggedIn = await deepseekPage
    .waitForSelector(DEEPSEEK_INPUT, { timeout: 8_000 })
    .then(() => true)
    .catch(() => false);

  if (alreadyLoggedIn) {
    console.log('[DEEPSEEK][Step 5] ✓ Already logged in to DeepSeek Chat.');
  } else {
    console.log('[DEEPSEEK][Step 5] DeepSeek login required...');

    const signInBtn = deepseekPage.locator(
    ...
    if (signInVisible) {
      console.log('[DEEPSEEK][Step 5] Clicking Sign in...');
      await signInBtn.click();
      await deepseekPage.waitForTimeout(2000);
    }

    console.log('[DEEPSEEK][Step 5] URL after click:', deepseekPage.url());
    console.log('[DEEPSEEK][Step 5]');
    console.log('[DEEPSEEK][Step 5] ════ ACTION REQUIRED ════════════════════════════════════');
    console.log('[DEEPSEEK][Step 5]  DeepSeek sign-in page is open in the Chrome window.');
    console.log('[DEEPSEEK][Step 5]  Please log in with your DeepSeek account.');
    console.log('[DEEPSEEK][Step 5]  Test continues automatically once the DeepSeek input is visible.');
    console.log('[DEEPSEEK][Step 5] ════════════════════════════════════════════════════════');
    console.log('[DEEPSEEK][Step 5] Waiting up to 2 minutes for DeepSeek to become ready...');

    await deepseekPage
      .waitForSelector(DEEPSEEK_INPUT, { timeout: 120_000 })
      .then(() => console.log('[DEEPSEEK][Step 5] ✓ DeepSeek login complete — input visible.'))
      .catch(() => console.log('[DEEPSEEK][Step 5] DeepSeek login not detected — tests may fail.'));
  }

  sharedPage = deepseekPage;

  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 6 — Verify extension is active on DeepSeek (__SENTINEL_INJECTED__)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [DEEPSEEK] STEP 6: Verifying extension active    │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  console.log(`[DEEPSEEK][Step 6] __SENTINEL_INJECTED__ : ${step6Injected}`);
  console.log(`[DEEPSEEK][Step 6] fetch() length        : ${fetchLen}`);
  if (sentinelBootLogs.length) console.log('[DEEPSEEK][Step 6] Sentinel logs:', sentinelBootLogs.slice(0, 3));
  else                         console.log('[DEEPSEEK][Step 6] No Sentinel console logs captured.');

  if (step6Injected) {
    console.log('[DEEPSEEK][Step 6] ✓ Extension is active on DeepSeek Chat.');
  } else {
    console.log('[DEEPSEEK][Step 6] ✗ __SENTINEL_INJECTED__ = false — content script not running.');
  }

  console.log('\n[DEEPSEEK][Setup] All steps complete — starting tests.\n');
});
...
// ---------------------------------------------------------------------------
// TEST 1 — Verify extension is active on DeepSeek Chat
// ---------------------------------------------------------------------------

test('[DeepSeek] 1 - Extension is active on DeepSeek Chat (__SENTINEL_INJECTED__)', async () => {
  ...
  console.log('\n[DEEPSEEK][Test 1] STEP 7 — Navigating to DeepSeek Chat...');
  await page.goto('https://chat.deepseek.com/', { waitUntil: 'commit', timeout: 90_000 });
  await page.waitForSelector(DEEPSEEK_INPUT, { timeout: 60_000 });
  ...
  console.log(`[DEEPSEEK][Test 1] STEP 8 — __SENTINEL_INJECTED__: ${injected}  |  fetch hooked: ${fetchHooked}`);
  if (sentinelLogs.length) console.log('[DEEPSEEK][Test 1] Sentinel logs:', sentinelLogs.slice(0, 5));

  expect(
    injected,
    '__SENTINEL_INJECTED__ is false — extension not active on DeepSeek Chat.',
  ).toBe(true);

  console.log('[DEEPSEEK][Test 1] PASS — Sentinel extension is active on DeepSeek Chat.');
});

// ---------------------------------------------------------------------------
// TEST 2 — Enter 28 prompts on DeepSeek → validate Sentinel decision
// ---------------------------------------------------------------------------

test('[DeepSeek] 2 - Sentinel intercepts prompt on DeepSeek (WARN / BLOCK / REDACT / ALLOW)', async () => {
  ...
  // Navigate to DeepSeek once for the SW-ready check
  await page.goto('https://chat.deepseek.com/', { waitUntil: 'commit', timeout: 90_000 });
  await page.waitForSelector(DEEPSEEK_INPUT, { timeout: 60_000 });
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    console.log(`[DEEPSEEK][Test 2] Waiting 30 s for Sentinel to initialise (attempt ${attempt}/3)...`);
    await page.waitForTimeout(30_000);
    swReady = await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__).catch(() => false);
    console.log(`[DEEPSEEK][Test 2] Sentinel SW ready: ${swReady}`);
    if (swReady) break;
    if (attempt < 3) {
      console.log('[DEEPSEEK][Test 2] SW not ready — reloading page to trigger policy re-sync...');
      await page.reload({ waitUntil: 'commit', timeout: 60_000 });
      await page.waitForSelector(DEEPSEEK_INPUT, { timeout: 30_000 }).catch(() => {});
    }
  }
  if (!swReady) {
    console.log('[DEEPSEEK][Test 2] WARNING: Sentinel SW still not ready after 3 attempts — results may be affected.');
  }
  ...
  async function typeAndSubmit(p: Page, text: string): Promise<boolean> {
    const inp = p.locator(DEEPSEEK_INPUT).first();
    ...
    try {
      const btn = p.locator(DEEPSEEK_SEND).first();
      ...
      console.log('[DEEPSEEK][Test 2] Submit error:', String(err).split('\n')[0]);
      return false;
    }
  }
  ...
  for (let i = 0; i < TEST_PROMPTS.length; i++) {
    ...
    console.log(`\n${'─'.repeat(60)}`);
    console.log(`[DEEPSEEK][Test 2] Prompt ${i + 1} / ${TEST_PROMPTS.length}`);
    console.log(`[DEEPSEEK][Test 2] "${prompt.substring(0, 70)}..."`);
    ...
    console.log(`[DEEPSEEK][Test 2] Sentinel active: ${await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__)}`);

    const submitted = await typeAndSubmit(page, prompt);
    if (!submitted) {
      console.log('[DEEPSEEK][Test 2] Could not submit — skipping prompt.');
      results.push({ index: i + 1, category, expected, decision: 'SKIP', detectedCategory: '', prompt, status: 'SKIP' });
      continue;
    }

    // Wait for Sentinel decision (Sentinel intercepts before DeepSeek responds)
    ...
    console.log(`[DEEPSEEK][Test 2] Prompt ${i + 1} → Decision: ${decision}  Det.Category: ${detectedCategory || '(none)'}  [${status}]`);
    ...
    if (decision === 'BLOCK') {
      console.log('[DEEPSEEK][Test 2] BLOCK detected — opening fresh conversation for next prompt...');
  ...
  reportLines.push(`  Guardrail Sentinel — DeepSeek Chat Test Report`);
  ...
  const reportFile = path.join(reportDir, `deepseek_${fileTs}.txt`);
  fs.writeFileSync(reportFile, reportText, 'utf8');
  console.log(`[DEEPSEEK][Test 2] Report saved → ${reportFile}`);
  console.log(`[DEEPSEEK][Test 2] Total Sentinel API calls: ${sentinelApiCalls.length}`);

  const extensionActive = await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__).catch(() => false);
  expect(extensionActive, 'Sentinel extension not active on DeepSeek Chat — content script not running.').toBe(true);

  console.log('[DEEPSEEK][Test 2] Keeping browser open for 60 s to review the Sentinel results...');
