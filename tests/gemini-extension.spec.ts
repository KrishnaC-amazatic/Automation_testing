/**
 * Guardrail Sentinel — Playwright Automation Test (Gemini)
 *
 * Mirrors sentinel-extension.spec.ts but targets Google Gemini instead of ChatGPT.
 * Shares the same 28 TEST_PROMPTS and produces a matching report file.
 *
 * Run standalone:  npx playwright test gemini --reporter=list
 * Run in parallel: npx playwright test --reporter=list   (workers=2 in config)
 */
...
const GEMINI_SEND =
  ...
  'button[data-tooltip="Send message"], button[aria-label="Submit"], ' +
...
test.beforeAll(async () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [GEMINI] STEP 1: Launching Chrome                │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  context.on('serviceworker', (worker) => {
    ...
    if (url.startsWith('chrome-extension://') && !extensionId) {
      ...
      console.log('[GEMINI][SW event] Extension ID:', extensionId);
    }
  });
  ...
  console.log('[GEMINI][Step 1] Chrome launched.');
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [GEMINI] STEP 2: Loading extension               │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  if (alreadyLoaded) {
    console.log('[GEMINI][Step 2] Extension already loaded — skipping Load Unpacked.');
  } else {
    console.log('[GEMINI][Step 2] Enabling Developer Mode...');
    ...
    console.log('[GEMINI][Step 2] Clicking "Load unpacked"...');
    ...
    if (!clicked) {
      console.log('[GEMINI][Step 2] WARNING: Could not find "Load unpacked" button.');
    } else {
      console.log('[GEMINI][Step 2] "Load unpacked" clicked — handling file dialog...');
      const psScript = [
      ...
      try {
        ...
        console.log('[GEMINI][Step 2] File dialog handled.');
      } catch (err) {
        console.log('[GEMINI][Step 2] File dialog error:', String(err).split('\n')[0]);
      } finally {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [GEMINI] STEP 3: Verifying extension & ID        │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  if (idFromDom) {
    ...
    console.log('[GEMINI][Step 3] ✓ Extension loaded — ID:', extensionId);
  } else if (extensionId) {
    console.log('[GEMINI][Step 3] ✓ Extension ID from SW event:', extensionId);
  } else {
    const sw = context.serviceWorkers().find(w => w.url().startsWith('chrome-extension://'));
    if (sw) {
      extensionId = sw.url().split('/')[2];
      console.log('[GEMINI][Step 3] ✓ Extension ID from SW list:', extensionId);
    } else {
      console.log('[GEMINI][Step 3] ✗ Extension NOT found in chrome://extensions.');
    }
  }
  ...
  if (!extensionId) {
    console.log('[GEMINI][Step 4] Skipped — extension ID unknown.');
  } else {
    const popupUrl = `chrome-extension://${extensionId}/popup/popup.html`;
    console.log('[GEMINI][Step 4] Opening popup:', popupUrl);
    try {
      ...
      console.log('[GEMINI][Step 4] ✓ Popup opened.');
      ...
      if (alreadySignedIn) {
        console.log('[GEMINI][Step 4] ✓ Already signed in.');
      } else {
        console.log('[GEMINI][Step 4] Not signed in — automating sign-in...');
        const popupPreview = await popupPage
          .evaluate(() => (document.body?.innerText ?? '').replace(/\n+/g, ' ').substring(0, 200))
          .catch(() => '');
        console.log('[GEMINI][Step 4] Popup content:', popupPreview);
        ...
        console.log('[GEMINI][Step 4] Clicked Sign In — waiting for auth tab...');
        ...
        try {
          ...
          console.log('[GEMINI][Step 4] Auth page URL:', authPage.url());
        } catch {
          authPage = popupPage;
          await authPage.waitForTimeout(3000);
          console.log('[GEMINI][Step 4] Auth in popup page:', authPage.url());
        }
        ...
        for (const sel of emailSelectors) {
          ...
          if (visible) {
            ...
            console.log('[GEMINI][Step 4] Filled email using selector:', sel);
            emailFilled = true;
            break;
          }
        }
        if (!emailFilled) {
          ...
          console.log('[GEMINI][Step 4] Filled email via JS evaluate (fallback).');
        }
        ...
        console.log('[GEMINI][Step 4] Credentials submitted — waiting for redirect...');
        ...
        console.log(signedIn
          ? '[GEMINI][Step 4] ✓ Signed in successfully.'
          : '[GEMINI][Step 4] ✗ Sign-in may have failed — Sentinel will run unauthenticated.');
      }
    } catch (err) {
      console.log('[GEMINI][Step 4] Error:', String(err).split('\n')[0]);
    }
    await popupPage?.close().catch(() => {});
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 5 — Log in to Gemini (Google account)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [GEMINI] STEP 5: Log in to Gemini                │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  console.log('[GEMINI][Step 5] Navigating to Gemini...');
  await geminiPage.goto('https://gemini.google.com/', { waitUntil: 'commit', timeout: 60_000 })
    .catch(e => console.log('[GEMINI][Step 5] Navigation error:', String(e).split('\n')[0]));

  // Allow any redirect to complete (Gemini may redirect to /app)
  await geminiPage.waitForTimeout(3000);
  console.log('[GEMINI][Step 5] Current URL:', geminiPage.url());

  // Check if already logged in (Gemini input visible)
  const alreadyLoggedIn = await geminiPage
    .waitForSelector(GEMINI_INPUT, { timeout: 8_000 })
    .then(() => true)
    .catch(() => false);

  if (alreadyLoggedIn) {
    console.log('[GEMINI][Step 5] ✓ Already logged in to Gemini.');
  } else {
    console.log('[GEMINI][Step 5] Gemini login required...');

    // Gemini may show a "Sign in" button or redirect automatically
    const signInBtn = geminiPage.locator(
      'a:has-text("Sign in"), button:has-text("Sign in"), ' +
      'a:has-text("Get started"), button:has-text("Get started"), ' +
      'a[href*="accounts.google.com"], a[href*="signin"]'
    ).first();

    const signInVisible = await signInBtn.isVisible({ timeout: 8_000 }).catch(() => false);
    if (signInVisible) {
      console.log('[GEMINI][Step 5] Clicking Sign in...');
      await signInBtn.click();
      await geminiPage.waitForTimeout(2000);
    }

    // At this point the user may be on accounts.google.com account picker
    console.log('[GEMINI][Step 5] URL after click:', geminiPage.url());
    console.log('[GEMINI][Step 5]');
    console.log('[GEMINI][Step 5] ════ ACTION REQUIRED ════════════════════════════════════');
    console.log('[GEMINI][Step 5]  Google sign-in page is open in the Chrome window.');
    console.log('[GEMINI][Step 5]  Please select your Google account to log in to Gemini.');
    console.log('[GEMINI][Step 5]  Test continues automatically once the Gemini input is visible.');
    console.log('[GEMINI][Step 5] ════════════════════════════════════════════════════════');
    console.log('[GEMINI][Step 5] Waiting up to 2 minutes for Gemini to become ready...');

    await geminiPage
      .waitForSelector(GEMINI_INPUT, { timeout: 120_000 })
      .then(() => console.log('[GEMINI][Step 5] ✓ Gemini login complete — input visible.'))
      .catch(() => console.log('[GEMINI][Step 5] Gemini login not detected — tests may fail.'));
  }

  sharedPage = geminiPage;

  // ═══════════════════════════════════════════════════════════════════════════
  // STEP 6 — Verify extension is active on Gemini (__SENTINEL_INJECTED__)
  // ═══════════════════════════════════════════════════════════════════════════
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│ [GEMINI] STEP 6: Verifying extension active      │');
  console.log('└─────────────────────────────────────────────────┘');
  ...
  console.log(`[GEMINI][Step 6] __SENTINEL_INJECTED__ : ${step6Injected}`);
  console.log(`[GEMINI][Step 6] fetch() length        : ${fetchLen}`);
  if (sentinelBootLogs.length) console.log('[GEMINI][Step 6] Sentinel logs:', sentinelBootLogs.slice(0, 3));
  else                         console.log('[GEMINI][Step 6] No Sentinel console logs captured.');

  if (step6Injected) {
    console.log('[GEMINI][Step 6] ✓ Extension is active on Gemini.');
  } else {
    console.log('[GEMINI][Step 6] ✗ __SENTINEL_INJECTED__ = false — content script not running.');
  }

  console.log('\n[GEMINI][Setup] All steps complete — starting tests.\n');
});
...
// ---------------------------------------------------------------------------
// TEST 1 — Verify extension is active on Gemini
// ---------------------------------------------------------------------------

test('[Gemini] 1 - Extension is active on Gemini (__SENTINEL_INJECTED__)', async () => {
  ...
  console.log('\n[GEMINI][Test 1] STEP 7 — Navigating to Gemini...');
  await page.goto('https://gemini.google.com/', { waitUntil: 'commit', timeout: 90_000 });
  await page.waitForSelector(GEMINI_INPUT, { timeout: 60_000 });
  ...
  console.log(`[GEMINI][Test 1] STEP 8 — __SENTINEL_INJECTED__: ${injected}  |  fetch hooked: ${fetchHooked}`);
  if (sentinelLogs.length) console.log('[GEMINI][Test 1] Sentinel logs:', sentinelLogs.slice(0, 5));

  expect(
    injected,
    '__SENTINEL_INJECTED__ is false — extension not active on Gemini.',
  ).toBe(true);

  console.log('[GEMINI][Test 1] PASS — Sentinel extension is active on Gemini.');
});

// ---------------------------------------------------------------------------
// TEST 2 — Enter 28 prompts on Gemini → validate Sentinel decision
// ---------------------------------------------------------------------------

test('[Gemini] 2 - Sentinel intercepts prompt on Gemini (WARN / BLOCK / REDACT / ALLOW)', async () => {
  ...
  // The SW can report "Offline (N failures)" if org policies haven't synced yet.
  // We allow up to 3 attempts: wait 30 s → check injection → reload if still not ready.
  let swReady = false;
  for (let attempt = 1; attempt <= 3; attempt++) {
    console.log(`[GEMINI][Test 2] Waiting 30 s for Sentinel to initialise (attempt ${attempt}/3)...`);
    await page.waitForTimeout(30_000);
    swReady = await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__).catch(() => false);
    console.log(`[GEMINI][Test 2] Sentinel SW ready: ${swReady}`);
    if (swReady) break;
    if (attempt < 3) {
      console.log('[GEMINI][Test 2] SW not ready — reloading page to trigger policy re-sync...');
      await page.reload({ waitUntil: 'commit', timeout: 60_000 });
      await page.waitForSelector(GEMINI_INPUT, { timeout: 30_000 }).catch(() => {});
    }
  }
  if (!swReady) {
    console.log('[GEMINI][Test 2] WARNING: Sentinel SW still not ready after 3 attempts — results may be affected.');
  }
  ...
  for (let i = 0; i < TEST_PROMPTS.length; i++) {
    ...
    console.log(`\n${'─'.repeat(60)}`);
    console.log(`[GEMINI][Test 2] Prompt ${i + 1} / ${TEST_PROMPTS.length}`);
    console.log(`[GEMINI][Test 2] "${prompt.substring(0, 70)}..."`);
    ...
    console.log(`[GEMINI][Test 2] Sentinel active: ${await page.evaluate(() => !!(window as any).__SENTINEL_INJECTED__)}`);

    // Focus and clear the Gemini input
    const input = page.locator(GEMINI_INPUT).first();
    await input.click();
    ...
    let submitted = false;
    try {
      const sendBtn = page.locator(GEMINI_SEND).first();
      ...
      console.log('[GEMINI][Test 2] Submit error:', String(err).split('\n')[0]);
    }
  ...
  expect(extensionActive, 'Sentinel extension not active on Gemini — content script not running.').toBe(true);

  console.log('[GEMINI][Test 2] Keeping browser open for 60 s to review the Sentinel results...');
