...
const EXTENSION_PATH = path.resolve(__dirname, '../extension-build');
...
test.beforeAll(async () => {
  ...
  // Credentials for Guardrail Sentinel
...
test('2 - Sentinel intercepts prompt (WARN / BLOCK / REDACT / ALLOW)', async () => {
  ...
  reportLines.push(`Guardrail Sentinel — Test Report  [${timestamp}]`);
  ...
  const reportFile = path.join(reportDir, `sentinel-chatgpt-report-${Date.now()}.txt`);
